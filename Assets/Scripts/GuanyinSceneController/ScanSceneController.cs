...
namespace Guanyin
{
	...
	public class ScanSceneController : MonoBehaviour
	{
		...
		void Awake ()
		{
			...
			cameraObjectContainer = ARCamera.gameObject.GetChildByName("ObjectContainer");
...
//			text.text = "gyro not supported";
		...
		{
			Logger.Log (GetAssetsPath("dabeizhou.mp3", true), "purple");
			...
			if (state == ScanSceneState.AnimationAfterName) {
				...
				if (screenTouch.doubleTouched) {
					...
					if (doubleDisChanged > 1.2f || doubleDisChanged < .8f) {
						...
						float s = guanyinTouchedScale * Mathf.Pow((doubleDisChanged > 1.2f ? (doubleDisChanged - .2f) : (doubleDisChanged + .2f)), 1.5f);
